...
class TestProgramComponents:
    ...
    def test_to_normalized_string_full(self):
        """to_normalized_string returns base name only (degree/honours/coop omitted)."""
    ...
    def test_to_normalized_string_with_specialization(self):
        """to_normalized_string should return base name only, excluding specialization."""
        comp = ProgramComponents(
        ...
        result = comp.to_normalized_string()
        assert result == "Mechanical Engineering"
        assert "Automotive" not in result

    def test_to_key_format(self):
        """to_key produces a lowercase base-name-only key."""
        comp = ProgramComponents(
            original="BSc Honours Computer Science Co-op",
            base_name="Computer Science",
            degree="BSc",
...
class TestProgramNormalizer:
    ...
    def test_normalize_full_string(self, prog_normalizer):
        """normalize should produce a pipe-separated standardised string (degree omitted)."""
        result = prog_normalizer.normalize("BSc Honours Computer Science Co-op")
        assert isinstance(result, str)
        assert "Computer Science" in result
        assert "BSc" not in result  # degree is omitted from normalized output
    ...
    def test_to_key_lowercase(self, prog_normalizer):
        """to_key should return a lowercase base-name-only string."""
        key = prog_normalizer.to_key("BSc Honours Computer Science Co-op")
        assert isinstance(key, str)
        assert key == key.lower()
        assert "computer_science" in key
...
class TestStubCoverage:
    ...
    def test_uni_load_mapping(self, universities_yaml):
        ...
        assert isinstance(result, dict)
        assert len(result) > 0
        # Keys should be canonical names, values should be lists of variations
    ...
    def test_uni_build_reverse_lookup(self, universities_yaml):
        ...
        assert "tu" in result
        assert result["test"] == "Test University"
        assert result["tu"] == "Test University"

    def test_uni_normalize_with_empty_mapping(self, universities_yaml):
        """Call normalize on a raw instance with empty mapping -- returns INVALID."""
        norm = UniversityNormalizer.__new__(UniversityNormalizer)
        norm.mapping = {}
        norm.reverse_lookup = {}
    ...
    def test_uni_normalize_none_input(self, uni_normalizer):
        """Normalize None should return INVALID."""
        result = uni_normalizer.normalize(None)
        assert result == "INVALID"

    def test_uni_normalize_empty_string(self, uni_normalizer):
        """Normalize empty string should return INVALID."""
        result = uni_normalizer.normalize("")
        assert result == "INVALID"

    def test_uni_get_variations(self, uni_normalizer):
        """get_variations returns a list of known variations."""
        result = uni_normalizer.get_variations("University of Toronto")
        assert isinstance(result, list)
        assert len(result) > 0

    def test_uni_get_variations_unknown(self, uni_normalizer):
        """get_variations for unknown university returns empty list."""
        result = uni_normalizer.get_variations("Nonexistent University XYZ")
        assert isinstance(result, list)
        assert len(result) == 0
    ...
    def test_prog_build_reverse_lookup(self):
        ...
        assert result["cs"] == "Computer Science"
    ...
    def test_prog_compile_patterns(self):
        ...
        assert "honours" in result
        assert "specialization" in result
        # Should have degree patterns
        assert any(k.startswith("degree_") for k in result)
    ...
    def test_prog_clean_base_name(self):
        ...
        assert isinstance(result, str)
        assert "Computer Science" in result
        # Should have removed separators and stripped whitespace
    ...
    def test_prog_normalize_base_name(self):
        ...
        assert isinstance(result, str)
        # No match in empty mapping, should return original
        assert result == "Computer Science"

    def test_prog_normalize_base_name_empty(self):
        """_normalize_base_name with empty string returns empty."""
        norm = ProgramNormalizer.__new__(ProgramNormalizer)
        norm.base_mapping = {}
        norm.reverse_lookup = {}
        norm.fuzzy_threshold = 80
        norm.stats = NormalizationStats()
        result = norm._normalize_base_name("")
        assert result == ""

    def test_prog_normalize_series(self, prog_normalizer):
        """Call ProgramNormalizer.normalize_series."""
        import pandas as pd
    ...
    def test_uni_normalize_series(self, uni_normalizer):
        """Call UniversityNormalizer.normalize_series."""
        import pandas as pd
        s = pd.Series(["UofT"])
        result = uni_normalizer.normalize_series(s)
        assert isinstance(result, pd.Series)
    ...
    def test_uni_get_report(self, uni_normalizer):
        """get_report returns a dict with stats after normalization."""
        uni_normalizer.normalize("UofT")
    ...
    def test_uni_get_canonical_names(self, uni_normalizer):
        """get_canonical_names returns a list of canonical names."""
        result = uni_normalizer.get_canonical_names()
        assert isinstance(result, list)
        assert len(result) > 0
        assert "University of Toronto" in result

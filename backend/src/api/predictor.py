...
@dataclass
class Activity:
    """A single extracurricular activity described in free text."""
    description: str
...
class PredictionResponse:
    ...
    ec_assessment: Optional[Any] = None
...
class PredictorService:
    ...
    def __init__(self,
        ...
        self._feature_names = [
            ...
            'is_ontario', 'is_bc', 'is_alberta', 'is_quebec', 'ec_score_weighted'
        ]
        self._beta = np.array([-0.5, 0.08, 0.02, 0.03, 0.1, 0.05, 0.05, 0.08, 0.04])
        self._cal_a = 1.0
        self._cal_b = 0.0
        ...
        self._ec_weights = self._load_ec_weights()

    # ── EC scoring ─────────────────────────────────────────────────────

    def _load_ec_weights(self) -> Dict[str, float]:
        """Load program-specific EC weight mappings from YAML."""
        import yaml
        import os
        path = os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'mappings', 'ec_weights.yaml')
        try:
            with open(path) as f:
        ...
        tier_map = {'high': 0.50, 'moderate': 0.15, 'none': 0.0}
        for tier_name, patterns in data.items():
            if tier_name in tier_map and isinstance(patterns, list):
                for pattern in patterns:
                    weights[pattern] = tier_map[tier_name]
        return weights

    def get_ec_weight(self, university: str, program: str) -> float:
        """Return the EC weight multiplier for a university-program pair."""
        import fnmatch
        for pattern, weight in self._ec_weights.items():
            parts = pattern.split('|', 1)
            if len(parts) != 2:
                continue
            uni_pat, prog_pat = parts
            if fnmatch.fnmatch(university, uni_pat) and fnmatch.fnmatch(program, prog_pat):
                return weight
        return 0.0  # default: ECs not considered

    def score_extracurriculars(
        self,
        ...
        """Score activities via semantic similarity against historical data.

        Returns an ECAssessment from ec_scorer, or None if no activities.
        """
        if not activities:
            return None
        descriptions = [
        ...
        if not descriptions:
            return None
        from src.api.ec_scorer import get_ec_scorer
        scorer = get_ec_scorer()
        return scorer.score(descriptions, university, program, grade)

    def find_similar_cases_for_ec(
        self,
        activities: Optional[List[Dict[str, Any]]],
        university: str,
        program: str,
        ...
        """Find similar historical applicants for scatter plot."""
        if not activities:
            return []
        descriptions = [
            a.get("description", "") for a in activities
        ...
        from src.api.ec_scorer import get_ec_scorer
        scorer = get_ec_scorer()
        cases = scorer.find_similar_cases(descriptions, university, program, grade, k)
        return [
            {
                "grade": c.grade,
                "ec_score": c.ec_score,
    ...
    def predict(self, request: ApplicationRequest) -> PredictionResponse:
        ...
        ec_assessment = self.score_extracurriculars(
            request.activities, request.university, request.program, request.top_6_average,
        )
        similar_cases = self.find_similar_cases_for_ec(
            request.activities, request.university, request.program, request.top_6_average,
        )
        x = self.build_features(request, ec_assessment)
    ...
    def build_features(self, request: ApplicationRequest,
                       ec_assessment: Optional[ECAssessment] = None) -> np.ndarray:
        """
        Build feature vector from application request.

        Args:
            request: Application request
            ec_assessment: Optional EC assessment result

        Returns:
            Feature vector as numpy array (9 features)
        """
        province = request.province or ''
        ec_weight = self.get_ec_weight(request.university, request.program)
        ec_score_norm = (ec_assessment.score / 20.0) if ec_assessment else 0.0
        features = [
            1.0,  # bias
            ...
            ec_score_norm * ec_weight,  # program-weighted EC contribution
        ]
    ...
    def calibrate(self, p_raw: float) -> float:
        ...
        ├─────────────────────────────────────────────────────────────┤
        │                                                              │
        │  Platt scaling fits a sigmoid to map raw model scores to    │
        │  calibrated probabilities via a linear transform in         │
        │  log-odds space:                                             │
        │                                                              │
        ...
        │  • a > 1.0: Raw predictions too uncertain (under-confident) │
        │  • a < 1.0: Raw predictions too confident (over-confident)  │
        ...
        z = self._cal_a * logit + self._cal_b
    ...
    def compute_confidence_interval(self,
        ...
        │  ────────────────────────────────────                        │
        │  Uses binomial proportion SE as a rough lower bound         │
        │  on uncertainty:                                             │
        │                                                              │
        │  SE = sqrt(p(1-p) / n_training)                              │
        │  CI = [p - 1.96*SE, p + 1.96*SE]                            │
        │                                                              │
        │  LIMITATION: This estimates uncertainty in the overall       │
        │  admission rate, NOT individual prediction uncertainty.      │
        │  True individual CIs require the Fisher information          │
        │  matrix (X^T W X)^{-1} from training, which is not          │
        │  currently stored in the model artifacts.                    │
        │                                                              │
        │  PROPER METHOD (delta method, future work):                  │
        │  Var(z) = x^T (X^T W X)^{-1} x                              │
        │  SE(z) = sqrt(Var(z))                                       │
        │  p_lower = sigmoid(z - 1.96*SE(z))                          │
        │  p_upper = sigmoid(z + 1.96*SE(z))                          │
        │                                                              │
        └─────────────────────────────────────────────────────────────┘
        ...
        return ConfidenceInterval(lower=round(lower, 4), upper=round(upper, 4), method="approximate")
...
_predictor = PredictorService(
    ...
    embedding_path="models/trained/embeddings.pkl",
...
app = FastAPI(
    title="Grade Prediction API",
    version="1.0.0",
    description="Predict Canadian university admission probabilities",
)
...
@app.get("/health")
def health():
    return health_endpoint(_predictor)


@app.get("/model/info")
def model_info():
    return model_info_endpoint(_predictor)


@app.post("/predict")
def predict(request: ApplicationRequest):
    try:
        return predict_endpoint(request, _predictor)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def predict_batch(request: BatchPredictionRequest):
    return predict_batch_endpoint(request, _predictor)
...
from src.api.analytics import get_program_analytics, get_program_listing


@app.get("/programs/analytics")
def program_analytics_query(university: str, program: str):
    """Get historical analytics for a specific university-program pair (query params)."""
    result = get_program_analytics(university, program)
    if result is None:
        raise HTTPException(
            status_code=404,
            detail=f"No data found for {program} at {university}",
        )
    return result


@app.get("/programs/{university}/{program}/analytics")
def program_analytics(university: str, program: str):
    """Get historical analytics for a specific university-program pair."""
    result = get_program_analytics(university, program)
    if result is None:
        raise HTTPException(
            status_code=404,
...
@app.get("/programs/listing")
def program_listing():
    """Get all university-program pairs with summary stats."""
    return get_program_listing()

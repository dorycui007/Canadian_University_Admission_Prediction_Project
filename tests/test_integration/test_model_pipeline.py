...
All source model classes (LogisticModel, BaselineModel) are currently stubs,
so every test calls ``pytest.skip`` until the implementations land.  The
tests document the expected contract so they can be un-skipped one-by-one
...
class TestModelTrainingPipeline:
    ...
    def test_logistic_fit_predict(self, rng):
        ...
        if result is None:
            pytest.skip("LogisticModel.fit not yet implemented")
    ...
    def test_baseline_fit_predict(self, rng):
        ...
        result = model.fit(X, y, program_ids=programs)
        if result is None:
            pytest.skip("BaselineModel.fit not yet implemented")
        assert model.is_fitted
